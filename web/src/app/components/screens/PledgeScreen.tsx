...
interface PledgeScreenProps {
  ...
  onNext: (nickname: string, pledge: string) => void;
  onBack: () => void;
}
...
export function PledgeScreen({ resultType, character, onNext, onBack }: PledgeScreenProps) {
  ...
  // 캐릭터 설정이 없으면 유형별 기본값(F) 사용
  const char = character ?? {
    gender: "F" as const,
    ...DEFAULT_CHARACTER[resultType].F,
  };
  ...
  return (
    <div className="relative min-h-full pt-12 pb-28 px-6">
      ...
      {/* Nickname (선택) */}
      <div className="mt-6 bg-white rounded-2xl p-5 shadow-[0_8px_24px_rgba(30,30,30,0.06)]">
        ...
        <input
          ...
          value={nickname}
          onChange={(e) => {
            if (e.target.value.length <= maxNickname) setNickname(e.target.value);
          }}
      ...
      <div className="mt-4 bg-white rounded-2xl p-5 shadow-[0_8px_24px_rgba(30,30,30,0.06)]">
        ...
        <textarea
          value={pledge}
          onChange={(e) => {
            if (e.target.value.length <= maxPledge) setPledge(e.target.value);
          }}
      ...
      {/* Examples */}
      <div className="mt-5">
        <div className="text-[#1E1E1E]/60 text-[12px] mb-2">예시 (클릭하면 입력돼요)</div>
      ...
      <div className="absolute bottom-6 left-6 right-6">
        <button
          ...
          onClick={() => onNext(nickname, pledge)}
